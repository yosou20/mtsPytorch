...
for i in range(valid_start, test_end): # range from test start to the end
	...
	#select_matrix_error
	#df_temp = select_matrix_error[select_matrix_error > thred_b]
	...
	num_broken = len(select_matrix_error[select_matrix_error > upper_bound31.max()*0.02 ]) #0.0055
	num_broken2 =len(df_temp)	
...
threshold = np.full((test_num), valid_anomaly_max)
axes.plot(threshold, color = 'green', linestyle = '--',linewidth = 2)
...
plt.title("MSCRED", size = 16)
plt.savefig('../outputs/anomaly_score.jpg')
plt.show()

def deep_anomaly_detector(valid_start, test_end, tfactor =1.5):
    abnormal_points1 = []
...
if __name__ == '__main__':
    df =deep_anomaly_detector(valid_start, test_end, 1.52) #1.98
